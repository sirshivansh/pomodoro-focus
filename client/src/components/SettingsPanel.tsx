    ...
    <div className="flex items-center justify-between gap-4 py-2">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {sub && <div className="text-xs mt-0.5" style={{ fontFamily: "'Space Grotesk',sans-serif", color: "rgba(255,255,255,0.42)" }}>{sub}</div>}
        </div>
      </div>
...
export default function SettingsPanel({ config, onSave, onClose }: SettingsPanelProps) {
  ...
  return (
    <div className="w-full max-w-sm rounded-2xl p-6 space-y-6"
      style={{
        background: "rgba(8,8,18,0.96)", border: "1px solid rgba(255,255,255,0.12)",
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold tracking-[0.06em]"
            style={{ fontFamily: "'Space Grotesk',sans-serif", color: "rgba(255,255,255,0.95)" }}>Settings</h2>
          <p className="text-xs font-semibold tracking-[0.2em] uppercase mt-0.5"
            style={{ fontFamily: "'Rajdhani',sans-serif", color: "rgba(255,255,255,0.4)" }}>Timer Configuration</p>
        </div>
      ...
      <div className="space-y-1">
        <ToggleRow icon={<Volume2 className="w-3.5 h-3.5" />}
          label="Sound" sub="Beep when sessions end"
          checked={s.soundEnabled} onChange={(v) => update("soundEnabled", v)} testId="switch-sound-enabled" />
        <ToggleRow icon={<SkipForward className="w-3.5 h-3.5" />}
          label="Auto-start" sub="Automatically begin next session"
          checked={s.autoStart} onChange={(v) => update("autoStart", v)} testId="switch-auto-start" />
        <ToggleRow icon={<Bell className="w-3.5 h-3.5" />}
          label="Notifications" sub="Browser alert when session ends"
          checked={s.notificationsEnabled} onChange={(v) => update("notificationsEnabled", v)} testId="switch-notifications" />
      </div>

      {/* Keyboard shortcuts hint */}
      <div className="rounded-xl px-4 py-3 space-y-1.5"
        style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="text-xs font-semibold tracking-[0.18em] uppercase mb-2"
        ...
        {[
          ...
          <div key={key} className="flex items-center justify-between">
            <span style={{ fontSize: "12px", fontFamily: "'Space Grotesk',sans-serif", color: "rgba(255,255,255,0.5)" }}>{label}</span>
